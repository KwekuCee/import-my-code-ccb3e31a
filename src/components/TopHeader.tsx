...
export const TopHeader: React.FC<TopHeaderProps> = ({
  ...
  return (
    <header className="bg-white/90 backdrop-blur-md border-b border-slate-200/90 w-full h-16 sticky top-0 z-30 px-4 md:px-8 flex items-center justify-between shadow-2xs">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-slate-100 border border-slate-200 text-slate-600 font-mono text-[10px] font-semibold rounded-md">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="relative hidden sm:block">
          <div className="relative flex items-center">
            ...
            {searchQuery ? (
              ...
              <kbd className="absolute right-2.5 text-[10px] font-mono font-semibold text-slate-400 bg-slate-200/60 px-1.5 py-0.5 rounded border border-slate-300/50">
                ⌘K
              </kbd>
            )}
          </div>
          ...
          {showSearchResults && searchQuery.trim().length > 0 && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              <div className="px-3 py-2 border-b border-slate-100 font-mono text-[10px] font-bold uppercase text-slate-400 flex justify-between bg-slate-50">
                ...
                filteredMembers.map((m) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="font-mono text-[10px] text-slate-500 mt-0.5">{m.id} • {m.role} • {m.church}</p>
                    </div>
            ...
            <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              <div className="p-3 bg-blue-800 text-white flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-xs uppercase tracking-wider">Notifications</h3>
                </div>
                <span className="text-[10px] font-mono bg-blue-600 text-white font-bold px-2 py-0.5 rounded-full">3 NEW</span>
              </div>
        ...
        <div className="flex items-center gap-2.5 pl-2 border-l border-slate-200">
          ...
          <div className="hidden lg:block text-left">
            ...
            <p className="font-mono text-[10px] text-blue-700 font-semibold leading-tight">{user.role}</p>
          </div>
        </div>
      </div>
    </header>
